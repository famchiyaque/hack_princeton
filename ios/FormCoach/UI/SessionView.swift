...
struct SessionView: View {
    ...
    // Set to true once the classifier first locks — prevents re-classification mid-session.
    ...
    @State private var coachMessage = "Curl, squat, or deadlift to begin"
    ...
    // MARK: - Exercise setup (called once when classifier locks)
    ...
    private func processFrame(_ pose: BodyPose) {
        ...
        // Phase 1: run classifier until the first stable exercise is detected.
        if !classifier.recognitionSuspended {
            classifier.update(angles: angles, pose: pose)
        }
        if !recognitionLocked, classifier.isStable, classifier.detectedExercise != .unknown {
        ...
        // Phase 2: starting-position gate — green flash on first lock.
        ...
        // Phase 3: curl elbow-stability gate (on top of position lock).
        if exercise == .curl, currentState == .locked {
            ...
            if exercise != .curl || currentState != .locked { jointHighlightStates = [:] }
        ...
        // Phase 4: rep counting — gated by position lock (+ curl elbow gate).
        ...
        switch exercise {
        case .deadlift, .jumpingJacks: peakAngle = max(peakAngle, angle)
        case .curl:                    peakAngle = min(peakAngle, angle)
        default:                       peakAngle = min(peakAngle, angle)
        }
    ...
    private func throttleCoachBubble(positionState: PositionState, result: FormResult, exercise: ExerciseType) {
        ...
        case .waiting:    coachMessage = exercise.startingPositionCue
        case .approaching: coachMessage = "Hold still… almost there"
        case .locked:     coachMessage = scheduler.visualHint(result: result)
        }
    }
    ...
    /// Shows "READY" before position is locked so there's no pre-lock GOOD/FIX flicker.
    private var formStatus: (label: String, color: Color) {
        guard recognitionLocked, posDetector.state == .locked else {
    ...
    private var exerciseStatus: (label: String, color: Color, icon: String) {
        ...
        case .squat:    return ("SQUAT",      KineticColor.success,       "figure.cooldown")
        case .deadlift: return ("DEADLIFT",   KineticColor.success,       "figure.strengthtraining.functional")
        case .curl:     return ("BICEP CURL", KineticColor.success,       "dumbbell.fill")
        case .unknown where classifier.isStable:
                        return ("NOT FOUND",  KineticColor.danger,        "questionmark")
        default:        return ("DETECTING…", KineticColor.textSecondary, "sparkle.magnifyingglass")
        }
    }
